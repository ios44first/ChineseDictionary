...
-(id)initWithFrame:(CGRect)frame superView:(UIView *)superView andiFlySpeechRecognizer:(IFlyRecognizerView *)speechRecgonizer
{
    self = [super initWithFrame:frame];
    if (self) {
        self.speechRecgonizer=speechRecgonizer;
        // Initialization code
        //初始化为NO
        keyboardIsShow=NO;
        ...
        //可以自适应高度的文本输入框
        self.textView = [[UIExpandingTextView alloc] initWithFrame:CGRectMake(40, 5, 210, 36)];
...
{
//    NSLog(@"文本的长度%d",self.textView.text.length);
...
//        NSLog(@"点击发送");
        ...
        {
            [delegate sendTextAction:self.textView.text];
        }
...
//    NSLog(@"进代理了");
    ...
    if ([str isEqualToString:@"删除"]) {
        if (self.textView.text.length>0) {
//            NSLog(@"dddddd=%d",self.textView.text.length-2);
            
            if (self.textView.text.length-1==0)
            {
                newStr=@"";
            }
            else if((self.textView.text.length-2)==0)
            {
